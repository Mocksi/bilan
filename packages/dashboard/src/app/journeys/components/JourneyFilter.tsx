...
import { JourneyFilterState } from '@/lib/types'
import { Search, Filter, X, Calendar, Users, Route, Star, Tag, Target } from 'lucide-react'

interface JourneyFilterProps {
  filters: JourneyFilterState
  onFiltersChange: (filters: JourneyFilterState) => void
  userOptions: string[]
  journeyNameOptions: string[]
  tagOptions: string[]
}
...
export function JourneyFilter({ 
  ...
  const hasActiveFilters = 
    ...
    filters.journeyName ||
    filters.status !== 'all' ||
    filters.minSteps !== null ||
    filters.maxSteps !== null ||
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 space-y-4">
      ...
      <div className="relative">
        ...
        <input
          type="text"
          placeholder="Search journeys, users, or steps..."
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          <select
            value={filters.journeyName}
            onChange={(e) => handleFilterChange('journeyName', e.target.value)}
            className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
          >
            <option value="">All journeys</option>
            {journeyNameOptions.map(name => (
        ...
        <div className="space-y-4 pt-4 border-t border-gray-200">
            ...
            <div className="grid grid-cols-2 gap-2">
              <div>
                <input
                  ...
                  onChange={(e) => handleFilterChange('minSteps', e.target.value ? parseInt(e.target.value) : null)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
              <div>
                <input
                  ...
                  onChange={(e) => handleFilterChange('maxSteps', e.target.value ? parseInt(e.target.value) : null)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                />
              </div>
            </div>
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => handleFilterChange('minCompletionRate', e.target.value ? parseFloat(e.target.value) : null)}
              className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent"
            />
          </div>
        ...
        <div className="pt-4 border-t border-gray-200">
          <div className="flex items-center space-x-2 text-sm text-gray-600">
            ...
            <div className="flex flex-wrap gap-1">
              ...
              {filters.minSteps !== null && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded">
                  Min: {filters.minSteps} steps
                </span>
              )}
              {filters.maxSteps !== null && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded">
                  Max: {filters.maxSteps} steps
                </span>
              )}
              {filters.minCompletionRate !== null && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded">
                  Completion: ≥{(filters.minCompletionRate * 100).toFixed(0)}%
                </span>
              )}
              {filters.satisfactionScore !== null && (
                <span className="px-2 py-1 bg-blue-100 text-blue-800 rounded">
