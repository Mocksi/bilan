...
export function calculateBasicStats(events: VoteEvent[], trendConfig: TrendConfig = {}): BasicStats {
  if (events.length === 0) {
    return {
      totalVotes: 0,
      positiveRate: 0,
  ...
  const recentTrend = calculateTrend(events, trendConfig)
...
function calculateTrend(events: VoteEvent[], config: TrendConfig = {}): 'improving' | 'declining' | 'stable' {
  const {
    sensitivity = 0.1,
    timeWeightHours = 24,
    minSampleSize = 5,
...
export function calculatePromptStats(events: VoteEvent[], promptId: PromptId): PromptStats {
...
export class BasicAnalytics {
  static calculateBasicStats = calculateBasicStats
  static calculatePromptStats = calculatePromptStats
} 